...
def post_syncdb_append_userandpermission(sender, **kwargs):
    """
    1,update database structure
    2,import data
    3,create admin and permission and group
    """
    try:
        if not User.objects.filter(username='admin'):
            User.objects.create_superuser('admin', 'fae_service@zkteco.com', 'admin')
...
post_syncdb.connect(post_syncdb_append_userandpermission)


def getDefaultDept():
    """
        Get the default department; no, create
    """
    try:
...
class NestedDeptException(Exception):
    pass
...
class department(models.Model):
    ...
    parent = models.IntegerField(db_column='supdeptid', verbose_name=_('parent'), null=True, blank=True)
    ...
    def save(self):
        if department.objects.all().count() == 0 or not self.parent or department.objects.filter(
                DeptID=self.parent).count() > 0:
            if not self.parent or self.DeptID == 1:
                self.parent = 0
...
def updateLastReboot(iclocks):
    ...
    if ips:
        cache.set(last_reboot_cname, lastReboot, rebInterval * 60)
    return ips
...
def checkTime(t):
    ...
    if str(type(t)) == "<type 'datetime.time'>":
        return datetime.datetime(1899, 12, 30, t.hour, t.minute, t.second)
    if str(type(t)) == "<type 'datetime.date'>":
...
class iclock(models.Model):
    SN = models.CharField(_('serial number'), max_length=20, primary_key=True, help_text=_(
        'Should be set with the device, on the device, access management menu "Sys Info" / "Dev Info" / "Serial Num" could be found'))
    ...
    TransTimes = models.CharField(_('transfer time'), max_length=50, null=True, blank=True, default='00:00;14:05',
                                  help_text=_(
                                      'Setting device for a moment from the plane started to send checks to the new data server. Hh: mm (hours: minutes) format, with a number of time between the semicolon (;) separately'))
    TransInterval = models.IntegerField(_('interval'), default=1, help_text=_(
        'Device set for each interval to check how many minutes to send new data server'))
    LogStamp = models.CharField(_('trans record stamp'), max_length=20, null=True, blank=True, help_text=_(
        'Logo for the latest device to the server send the transactions timestamps'))
    OpLogStamp = models.CharField(_('trans OP stamp'), max_length=20, null=True, blank=True, help_text=_(
        'Marking device for the server to the employee data transfer as timestamps'))
    PhotoStamp = models.CharField(_('trans photo stamp'), max_length=20, null=True, blank=True,
                                  help_text=_('Marking device for the server to the picture transfer as timestamps'))
    Alias = models.CharField(_('Device Alias name'), max_length=20, help_text=_('Device of a name'))
    DeptID = models.ForeignKey('department', db_column='DeptID', verbose_name=DEPT_NAME, default=1, editable=True,
                               null=True)
    UpdateDB = models.CharField(_('update flag'), max_length=10, default='1111111100', blank=True, editable=False,
                                help_text=_('To identify what kind of data should be transfered to the server'))
    ...
    LockFunOn = models.SmallIntegerField(db_column='AccFun', default=0, blank=True, editable=False,
                                         help_text=_('Access Function'))
    TZAdj = models.SmallIntegerField(_('Timezone'), default=3, blank=False, editable=True,
                                     help_text=_('Timezone of the location'), choices=TIMEZONE_CHOICES)
...
if settings.PIN_WIDTH == 5:
    MAX_PIN_INT = 99999
else:
    if settings.PIN_WIDTH == 10:
        MAX_PIN_INT = 4294967294L
    else:
        if settings.PIN_WIDTH <= 1:
            MAX_PIN_INT = 999999999999999999999999L
        else:
...
class employee(models.Model):
    ...
    DeptID = models.ForeignKey(department, db_column='defaultdeptid', verbose_name=DEPT_NAME, default=1, editable=True,
                               null=True)
    ...
    SN = models.ForeignKey(iclock, db_column='SN', verbose_name=_('registration device'), null=True, blank=True,
                           editable=True)
    ...
    def save(self):
        ...
        if not self.id:
            ...
            if int(old_emp.PIN) != int(self.PIN):
    ...
    def GetCopyFields(self):
        return [
            'National', 'PostCode', 'Address', 'Gender']
...
def getNormalCard(card):
    ...
    try:
        num = int(str(card))
        card = '[%02X%02X%02X%02X%02X]' % (num & 255, num >> 8 & 255, num >> 16 & 255, num >> 24 & 255, num >> 32 & 255)
    except:
        pass
...
def getEmpCmdStr(emp):
    ...
    ret = 'DATA UPDATE USERINFO PIN=%s\t%s\t%s\t%s\t%s\t%s\tGrp=%s' % (emp.pin(),
                                                                       ...
                                                                       'Pri=%s' % (
                                                                               emp.Privilege and emp.Privilege or 0),
                                                                       emp.Password and 'Passwd=%s' % emp.Password or '',
                                                                       emp.Card and 'Card=%s' % getNormalCard(
                                                                           emp.Card) or '',
...
def createThumbnail(imgUrlOrg, imgUrl):
    try:
        ...
        try:
            ...
            if not new_width == 0:
                if not new_height == 0 and cur_width > cur_height:
...
class transaction(models.Model):
    ...
    sensorid = models.CharField(db_column='sensorid', verbose_name=u'Sensor ID', null=True, blank=True, max_length=5,
                                editable=False)
    ...
    class Meta:
        ...
        permissions = (('clearObsoleteData_transaction', 'Clear Obsolete Data'),)
...
def OpName(op):
    OPNAMES = {0: _('start up'), 1: _('shutdown'),
...
def AlarmName(obj):
    ALARMNAMES = {50: _('Door Close Detected'),
...
class devlog(models.Model):
    ...
    def __unicode__(self):
        return '%s, %s, %s' % (self.SN, self.OpTime.strftime('%y-%m-%d %H:%M'), self.OP)
...
class devcmds(models.Model):
    ...
    def __unicode__(self):
        return '%s, %s' % (self.SN, self.CmdCommitTime.strftime('%y-%m-%d %H:%M'))

    def save(self, raw=None):
        if self.SN and not self.CmdTransTime:
            deviceHasCmd(self.SN)
        return models.Model.save(self)
    ...
    def fileURL(self):
        if self.CmdContent.find('GetFile ') == 0:
            ...
            if self.CmdContent.find('Shell ') == 0:
                fname = 'shellout.txt'
            else:
                return ''
...
def isUpdatingFW(device):
    return devcmds.objects.filter(SN=device, CmdReturn__isnull=True, CmdContent__startswith='PutFile ',
                                  CmdContent__endswith='main.gz.tmp').count()
...
class messages(models.Model):
    ...
    def __unicode__(self):
        return unicode(u'%s[%s]: %s' % (
            self.get_MsgType_display(), self.StartTime, self.MsgContent and self.MsgContent[:40] and ''))

    class Admin:
        list_filter = [
            'StartTime', 'MsgType', 'MsgParam', 'DeptID']
...
def delOldRecords(model, field, days):
    ...
    cursor.execute('DELETE FROM %s WHERE %s < %%s' % (table, field),
                   [str(datetime.datetime.now() - datetime.timedelta(days))[0:19]])
    connection._commit()


def delAllRecords(model):
    table = model._meta.db_table
    cursor = connection.cursor()
    if table == 'userinfo':
        cursor.execute('DELETE template')
        connection._commit()
        cursor.execute('DELETE checkinout')
        connection._commit()
    cursor.execute('DELETE %s' % table)
    connection._commit()
...
def createDefautValue():
    sqls = ('ALTER TABLE departments ADD CONSTRAINT sdf DEFAULT 1 FOR supdeptid',
...
def checkAndCreateModelPermissions(appName):
    ...
    try:
        ...
        try:
            ...
            Permission(content_type=ct, codename='browse_' + Group.__name__.lower(),
...
def __mci_iter__(self):
    if self.field.empty_label is not None:
        yield (
            u'', self.field.empty_label)
    if self.field.cache_choices:
        if self.field.choice_cache is None:
            self.field.choice_cache = [self.choice(obj) for obj in self.queryset]
...
def upgradeDB():
    sqls = ('ALTER TABLE userinfo ADD AutoSchPlan int NULL', 'ALTER TABLE userinfo ADD MinAutoSchInterval int NULL',
            'ALTER TABLE userinfo ADD RegisterOT int NULL', 'ALTER TABLE userinfo ADD Image_id int NULL',
            'ALTER TABLE iclock ADD PhotoStamp varchar(20) NULL', 'ALTER TABLE iclock ADD FWVersion varchar(30) NULL',
            'ALTER TABLE iclock ADD FPCount varchar(10) NULL',
            'ALTER TABLE iclock ADD TransactionCount varchar(10) NULL',
            'ALTER TABLE iclock ADD UserCount varchar(10) NULL', 'ALTER TABLE iclock ADD MainTime varchar(20) NULL',
            'ALTER TABLE iclock ADD MaxFingerCount int NULL', 'ALTER TABLE iclock ADD MaxAttLogCount int NULL',
            'ALTER TABLE iclock ADD DeviceName varchar(30) NULL', 'ALTER TABLE iclock ADD AlgVer varchar(30) NULL',
            'ALTER TABLE iclock ADD FlashSize varchar(10) NULL',
            'ALTER TABLE iclock ADD FreeFlashSize varchar(10) NULL',
            'ALTER TABLE iclock ADD Language varchar(30) NULL', 'ALTER TABLE iclock ADD VOLUME varchar(10) NULL',
            'ALTER TABLE iclock ADD DtFmt varchar(10) NULL', 'ALTER TABLE iclock ADD IPAddress varchar(20) NULL',
            'ALTER TABLE iclock ADD IsTFT varchar(5) NULL', 'ALTER TABLE iclock ADD Platform varchar(20) NULL',
            'ALTER TABLE iclock ADD Brightness varchar(5) NULL', 'ALTER TABLE iclock ADD BackupDev varchar(30) NULL',
            'ALTER TABLE iclock ADD OEMVendor varchar(30) NULL', 'ALTER TABLE iclock ADD AccFun int NOT NULL DEFAULT 0',
            'ALTER TABLE iclock ADD TZAdj int NOT NULL DEFAULT 8',
            'ALTER TABLE checkinout ADD WorkCode VARCHAR(20) NULL',
            'ALTER TABLE checkinout ADD Reserved VARCHAR(20) NULL', 'ALTER TABLE iclock DROP COLUMN CheckInterval')
    batchSql(sqls)

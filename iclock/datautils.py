    ...
    if fieldName in ordFields:
        ...
        if '-' + fieldName in ordFields:
            ...
            if not remove:
                ...
                return ''
    return (
        (',').join(ordFields), sorted, desc)


class FieldNameMulti(object):
            ...
            if f.name == fieldName:
                ...
                if self.orderUrl:
                    if sorted:
                        ...
                        if removeOrderStr:
                            removeOrderStr = self.cl.get_query_string({ORDER_VAR: removeOrderStr}, [ORDER_VAR]).replace(
                                "'", "\\'").replace('"', '\\"')
                        else:
...
def get_filters(opts, request, params, dataModel):
    filter_specs = []
    try:
        if opts.admin and opts.admin.list_filter:
            filter_fields = [opts.get_field(field_name) for field_name in opts.admin.list_filter]
...
def QueryData(request, dataModel):
    ...
    if request.GET.has_key(SEARCH_VAR):
        if opts.admin.search_fields:
            for bit in search.split():
                or_queries = [models.Q(**{construct_search(field_name): bit}) for field_name in
                              opts.admin.search_fields]
        ...
        for i in (
                ALL_VAR, ORDER_VAR, ORDER_TYPE_VAR, SEARCH_VAR, IS_POPUP_VAR, PAGE_VAR, STATE_VAR, EXPORT_VAR,
                PAGE_LIMIT_VAR,
                TMP_VAR):
        ...
        if request.user.is_superuser or request.user.username == 'employee':

    ...
    if objs.count():
        ...
        if model == employee:
            ...
            if model == iclock:
                ...
                info = model.objects.values(*copyFields)[0]
...
def DataExport(request, dataModel, qs, format):
    ...
    if '_' in extname:
        ...
        header = ''
    if not template:
        return render_to_response('info.html', {'title': _('Export data'), 'content': _(
            'Specified data format does not exist or do not support!')})
    if template[-1] == '\n':
        template = template[:-1]
    ...
    if 'csv' in extname:
        ...
        response['Content-Type'] = 'application/octet-stream; charset=' + settings.NATIVE_ENCODE
        response.write('\xef\xbb\xbf')
    ...
    response['Content-Length'] = len(content) + 3

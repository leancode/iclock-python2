...
def IEVersion(request):
    bVer = request.META['HTTP_USER_AGENT']
    p = bVer.find('MSIE ')
    if p < 0:
        return 100
...
class LocaleMiddleware(object):
    ...
    def process_request(self, request):
        ...
        if 'employee' in request.session:
            request.employee = request.session['employee']
...
def employee(request):
    ...
    if user.is_anonymous():
        return ''
    return user.username == 'employee' and (
        _(u'Staff %s') % request.session['employee'] if 'employee' in request.session else (
...
def myContextProc(request):
    ...
    return {'IS_ADMIN': emp == None and not user.is_anonymous(), 'login_username': userName,
            'employee': emp,
            'request': request}
